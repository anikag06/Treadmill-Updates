...
import { TREADWILL_MODULES } from '@/app.constants';
...
export class ModulesService {
    ...
    getModulesObservable(): Observable<Module[]> {
        const myFakeObservable = Observable.create((observer: Observer<Module[]>) => {
            ...
            let fakeModulesLs = this.localStorageService.getItemWithDate(TREADWILL_MODULES);
            if (fakeModulesLs) {
                ...
                setTimeout(() => {
                    observer.next(fakeModules)
                    observer.complete()
                    this.localStorageService.setItemWithDate(TREADWILL_MODULES, fakeModules);
                }, 1000);
            }
        })
    ...
    getModuleObservable(slug: string) {
        const myFakeObservable = Observable.create((observer: Observer<Module>) => {
            let module: any;
            let modulesLS = this.localStorageService.getItemWithDate(TREADWILL_MODULES)
            if (modulesLS) {
