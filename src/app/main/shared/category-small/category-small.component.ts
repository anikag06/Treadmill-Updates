...
export class CategorySmallComponent implements OnInit {
  ...
  imageUrl(): string {
    if (
      this.category.name.toUpperCase() === INTRODUCTION &&
      this.category.is_active === true
    ) {
      return 'assets/modules/introduction-yellow.svg';
    } else if (
      this.category.name.toUpperCase() === LEARN &&
      this.category.is_active === true
    ) {
      return 'assets/modules/learn-yellow.svg';
    } else if (
      this.category.name.toUpperCase() === DISCUSS &&
      this.category.is_active === true
    ) {
      return 'assets/modules/discuss-yellow.svg';
    } else if (
      this.category.name.toUpperCase() === PRACTICE &&
      this.category.is_active === true
    ) {
      return 'assets/modules/practice-yellow.svg';
    } else if (
      this.category.name.toUpperCase() === INTRODUCTION &&
      this.category.is_content_locked === true
    ) {
      ...
      this.category.name.toUpperCase() === INTRODUCTION &&
      this.category.is_content_locked === false &&
