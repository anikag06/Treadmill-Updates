...
export class ModifyBeliefsComponent implements OnInit {
  ...
  ngOnChanges(){
    if(this.worry){
      this.worryService.getModifyBeliefs(this.worry.id).subscribe(
        (resp : any) => {
          console.log('Modify Beliefs'+resp);
          if(resp.body.length !== 0){
            this.beliefForm.controls['beliefStatement'].setValue(resp.body.belief);
  ...
  onBeliefSubmit() {
    ...
    if(this.responseData.length == 0 && this.modifyBeliefs.length == 0){
      ...
      this.worryService.postModifyBeliefs(object).subscribe(
        (resp : any) => {
      ...
      this.worryService.putModifyBeliefs(object, this.worry.id).subscribe(
        (resp : any) => {
          const status = resp.ok;
          if (status) {
            console.log('The request has been submited');
